...
import 'package:flutter_mvvm_riverpod/features/matchmaking/ui/state/matchmaking_state_v2.dart';
...
class MessagesScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: trips.isEmpty ? const _EmptyMessages() : ListView.separated(
        ...
        itemBuilder: (context, index) {
          if (index == 0) {
            return const Padding(
              ...
              child: Text('Your trip groups', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
            );
          }
  ...
  List<MatchmakingTrip> _groupTrips(MatchmakingStateV2 state) {
    final acceptedIds = state.requests
        .where((request) => request.applicantId == 'current-user' && request.decision == ApplicantDecision.accepted)
        .map((request) => request.tripId)
        .toSet();
    return state.trips.where((trip) => trip.isOwned || acceptedIds.contains(trip.id)).toList(growable: false);
  }
}
  ...
  Widget build(BuildContext context) => Card(
    ...
    child: ListTile(
      leading: CircleAvatar(
        ...
        child: Text(trip.hostInitials.isEmpty ? trip.destination.substring(0, 1) : trip.hostInitials.substring(0, 1)),
      ),
      title: Text(trip.destination, style: const TextStyle(fontWeight: FontWeight.w700)),
      subtitle: Text('${_date(trip.startDate)} – ${_date(trip.endDate)} • ${trip.joined} travellers'),
  ...
  Widget build(BuildContext context) => Center(
    child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Icon(Icons.chat_bubble_outline_rounded, size: 56, color: AppColors.brandSurface),
        const SizedBox(height: 16),
        Text('No trip chats yet', style: AppTheme.title20),
        const SizedBox(height: 8),
        Text('Trips you create or join will appear here.', textAlign: TextAlign.center, style: AppTheme.body16.copyWith(color: AppColors.brandTextMuted)),
      ]),
    ),
  );
}
