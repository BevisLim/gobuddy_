...
class FriendManagementSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<FriendManagementSheet> createState() =>
      _FriendManagementSheetState();
}

class _FriendManagementSheetState extends ConsumerState<FriendManagementSheet>
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  ...
                  _Requests(state: state, viewModel: viewModel),
                  _InviteFriends(state: state, viewModel: viewModel),
                ],
              ),
            ),
          ],
...
class _FindFriendsState extends State<_FindFriends> {
  ...
  Widget build(BuildContext context) {
    ...
    final results = widget.state.friendDirectory
        .where(
          (friend) =>
              query.isEmpty ||
      ...
      children: [
          ...
          children: [
            Expanded(
              child: OutlinedButton.icon(
                onPressed: () => _showQr(context, title: 'Scan Friend QR Code'),
            ...
            Expanded(
              child: OutlinedButton.icon(
                onPressed: () => _showQr(context, title: 'My QR Code'),
        ...
        OutlinedButton.icon(
          onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Phone contacts synced. Matching GoBuddy users are shown below.',
              ),
            ),
          ),
        ...
        ...results.map((friend) {
          final sent = widget.state.sentRequestIds.contains(friend.id);
          return Card(
            child: ListTile(
              ...
              trailing: sent
                  ...
                  : FilledButton(
                      onPressed: () =>
                          widget.viewModel.sendFriendRequest(friend.id),
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                OutlinedButton(
                  onPressed: () => viewModel.declineFriendRequest(friend.id),
                  child: const Text('Decline'),
                ),
                const SizedBox(width: 6),
                FilledButton(
                  onPressed: () => viewModel.acceptFriendRequest(friend),
    ...
    children: [
      const Padding(
        ...
        child: Align(
          ...
          child: Text(
            'Select friends to invite to Tokyo Travel Group',
      ...
      Expanded(
        child: ListView(
          children: state.friendDirectory
              .map(
                (friend) => CheckboxListTile(
                  value: state.selectedFriendIds.contains(friend.id),
                  onChanged: (_) => viewModel.toggleFriendSelection(friend.id),
        ...
        child: SizedBox(
          ...
          child: FilledButton.icon(
            onPressed: state.selectedFriendIds.isEmpty
                ? null
                : () {
                    viewModel.inviteSelectedFriends();
                    Navigator.pop(context);
                  },
            icon: const Icon(Icons.group_add_outlined),
            label: Text(
              'Invite Selected Friends to Group (${state.selectedFriendIds.length})',
            ),
          ),
        ),
      ),
...
void _showQr(BuildContext context, {required String title}) => showDialog<void>(
  context: context,
  builder: (_) => AlertDialog(
    ...
    content: const Column(
      ...
      children: [
        ...
        SizedBox(height: 8),
        Text(
          'Use the camera scanner in the mobile app to connect with a friend.',
