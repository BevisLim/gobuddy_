...
class _PreviewTripDetails {
  const _PreviewTripDetails({required this.id, required this.destination, required this.title, required this.dates, required this.travellers});
  ...
  factory _PreviewTripDetails.fromTripId(String tripId) {
    switch (tripId.toLowerCase()) {
      case 'bali':
        return const _PreviewTripDetails(id: 'bali', destination: 'Bali, Indonesia', title: 'Bali Adventure', dates: 'Sep 5–15, 2026 • 10 days', travellers: '3 travellers • Planning together');
      case 'paris':
        return const _PreviewTripDetails(id: 'paris', destination: 'Paris, France', title: 'Paris Getaway', dates: 'Oct 2–6, 2026 • 5 days', travellers: '2 travellers • Planning together');
      case 'kyoto':
        return const _PreviewTripDetails(id: 'kyoto', destination: 'Kyoto, Japan', title: 'Kyoto Culture Trip', dates: 'Nov 12–16, 2026 • 5 days', travellers: '4 travellers • Planning together');
      default:
        return const _PreviewTripDetails(id: 'tokyo', destination: 'Tokyo, Japan', title: 'Tokyo Adventure', dates: 'Aug 10–12, 2026 • 3 days', travellers: '3 travellers • Planning together');
    }
  }
...
class GroupCollaborationPreviewScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('${trip.destination} Travel Group'), const Text('Group Communication & Collaboration', style: TextStyle(fontSize: 12))]),
          actions: [
        ...
        body: TabBarView(children: [_ChatTab(state: state, viewModel: viewModel), _TimelineTab(state: state, viewModel: viewModel, trip: trip), _FilesTab(state: state, viewModel: viewModel)]),
      ),
    );
  }
}
...
class _TimelineTab extends StatelessWidget {
  const _TimelineTab({required this.state, required this.viewModel, required this.trip});
  final CollaborationPreviewState state;
  final CollaborationPreviewViewModel viewModel;
  ...
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(16), children: [
    ...
    SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [for (var day = 1; day <= 3; day++) Padding(padding: const EdgeInsets.only(right: 8), child: ChoiceChip(label: Column(mainAxisSize: MainAxisSize.min, children: [Text('Day $day', style: const TextStyle(fontWeight: FontWeight.bold)), Text(trip.dayDate(day), style: const TextStyle(fontSize: 11))]), selected: state.selectedDay == day, selectedColor: const Color(0xFF149B8A), labelStyle: TextStyle(color: state.selectedDay == day ? Colors.white : const Color(0xFF293840)), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), onSelected: (_) => viewModel.selectDay(day)))])),
    const SizedBox(height: 12),
    Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10), decoration: BoxDecoration(color: const Color(0xFFF3F7F6), borderRadius: BorderRadius.circular(12)), child: Row(children: [Expanded(child: Text(trip.dayTitle(state.selectedDay), style: const TextStyle(fontWeight: FontWeight.w800))), TextButton.icon(onPressed: () => _showActivityForm(context, viewModel, proposal: true), icon: const Icon(Icons.add, size: 16), label: const Text('Add activity'))])),
    const SizedBox(height: 8),
    ..._activitiesForTrip(trip, state).asMap().entries.map((entry) { final key = _activityKey(trip.id, state.selectedDay, entry.key, entry.value.title); final activity = state.activityOverrides[key] ?? entry.value; return _activityCard(context, activity, state.pinnedActivityKeys.contains(key), viewModel, activityKey: key); }),
    if (state.selectedDay == 1) ...state.proposals.asMap().entries.map((entry) { final key = _activityKey(trip.id, 1, entry.key + 100, entry.value.title); final activity = state.activityOverrides[key] ?? entry.value; return _activityCard(context, activity, state.pinnedActivityKeys.contains(key), viewModel, activityKey: key); }),
  ...
  Widget _activityCard(BuildContext context, PreviewActivity activity, bool pinned, CollaborationPreviewViewModel viewModel, {required String activityKey}) => Card(child: ListTile(
    ...
    trailing: Wrap(spacing: 0, children: [
      IconButton(onPressed: () => _confirmPin(context, viewModel, activityKey), icon: Icon(pinned ? Icons.push_pin : Icons.push_pin_outlined), tooltip: 'Pin activity'),
      IconButton(onPressed: activity.isLocked ? null : () => _showActivityForm(context, viewModel, existing: activity, activityKey: activityKey), icon: const Icon(Icons.edit_outlined), tooltip: 'Edit activity'),
      IconButton(onPressed: () => viewModel.setActivityLock(activityKey, activity, !activity.isLocked), icon: Icon(activity.isLocked ? Icons.lock : Icons.lock_open_outlined), tooltip: 'Lock activity (admin)'),
    ]),
  ));
...
List<PreviewActivity> _activitiesForTrip(_PreviewTripDetails trip, CollaborationPreviewState state) {
  if (trip.id == 'bali') {
    return switch (state.selectedDay) {
      1 => const [
          PreviewActivity(title: 'Arrive at Ngurah Rai Airport', category: 'Flight', location: 'Denpasar', time: '9:00 AM', budget: 0, notes: 'Meet at the arrival gate.', status: 'Confirmed'),
          PreviewActivity(title: 'Check-in: The Layr Villa', category: 'Accommodation', location: 'Canggu', time: '12:00 PM', budget: 220, notes: 'Keep passports ready for check-in.', status: 'Confirmed'),
          PreviewActivity(title: 'Seminyak Beach walk + lunch', category: 'Dining', location: 'Seminyak', time: '2:00 PM', budget: 55, notes: 'Beachfront lunch and sunset walk.', status: 'Proposed'),
        ],
      2 => const [
          PreviewActivity(title: 'Tegalalang Rice Terrace', category: 'Sightseeing', location: 'Ubud', time: '9:00 AM', budget: 25, notes: 'Bring water and comfortable shoes.', status: 'Confirmed'),
          PreviewActivity(title: 'Tegenungan Waterfall', category: 'Sightseeing', location: 'Gianyar', time: '1:30 PM', budget: 20, notes: 'Swimming is optional.', status: 'Confirmed'),
          PreviewActivity(title: 'Ubud market dinner', category: 'Dining', location: 'Ubud', time: '6:30 PM', budget: 60, notes: 'Vote for restaurant in the poll.', status: 'Proposed'),
        ],
      _ => const [
          PreviewActivity(title: 'Surf lesson', category: 'Sightseeing', location: 'Seminyak Beach', time: '9:00 AM', budget: 90, notes: 'Beginner lesson, equipment included.', status: 'Confirmed'),
          PreviewActivity(title: 'Tanah Lot sunset', category: 'Sightseeing', location: 'Beraban', time: '4:30 PM', budget: 15, notes: 'Leave before traffic gets busy.', status: 'Confirmed'),
          PreviewActivity(title: 'Farewell dinner', category: 'Dining', location: 'Canggu', time: '7:30 PM', budget: 80, notes: 'Final group dinner.', status: 'Proposed'),
        ],
    };
  }
  return switch (state.selectedDay) {
    1 => [state.activity],
    2 => const [PreviewActivity(title: 'Senso-ji Temple', category: 'Sightseeing', location: 'Asakusa', time: '9:30 AM', budget: 0, notes: 'Meet at hotel lobby.', status: 'Confirmed'), PreviewActivity(title: 'Ramen lunch', category: 'Dining', location: 'Shibuya', time: '1:00 PM', budget: 45, notes: 'Group reservation for four.', status: 'Proposed')],
    _ => const [PreviewActivity(title: 'Hotel check-out', category: 'Accommodation', location: 'Shinjuku', time: '10:00 AM', budget: 0, notes: 'Leave luggage at reception.', status: 'Confirmed'), PreviewActivity(title: 'Flight home', category: 'Flight', location: 'Narita Airport', time: '4:45 PM', budget: 0, notes: 'Check passport and boarding pass.', status: 'Confirmed')],
  };
}
...
class _TripSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [const Icon(Icons.flight_takeoff, color: Colors.white), const SizedBox(width: 8), Expanded(child: Text(trip.title, style: const TextStyle(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w800))), FilledButton.tonal(onPressed: () => _showShareTripDialog(context, trip), style: FilledButton.styleFrom(backgroundColor: Colors.white24, foregroundColor: Colors.white), child: const Text('Share'))]),
      const SizedBox(height: 4),
      Text(trip.dates, style: const TextStyle(color: Colors.white70)),
      const SizedBox(height: 12),
      Row(children: [const CircleAvatar(radius: 13, child: Text('T')), const SizedBox(width: 4), const CircleAvatar(radius: 13, child: Text('A')), const SizedBox(width: 8), Text(trip.travellers, style: const TextStyle(color: Colors.white70, fontSize: 12))]),
    ]),
  );
}

Future<void> _showShareTripDialog(BuildContext context, _PreviewTripDetails trip) => showDialog<void>(
  context: context,
  builder: (dialogContext) => AlertDialog(
    title: const Row(children: [Icon(Icons.ios_share_outlined), SizedBox(width: 10), Text('Share trip')]),
    content: SizedBox(width: 420, child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(trip.title, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 18)),
      const SizedBox(height: 4),
      Text('${trip.dates} • ${trip.destination}'),
      const SizedBox(height: 16),
      const Text('Invite link', style: TextStyle(fontWeight: FontWeight.w700)),
      const SizedBox(height: 6),
      Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.black12, borderRadius: BorderRadius.circular(10)), child: SelectableText(trip.shareLink)),
      const SizedBox(height: 12),
      const Text('Anyone you invite can view the itinerary and join the group conversation.', style: TextStyle(fontSize: 13)),
    ])),
    actions: [
      TextButton.icon(onPressed: () async { await Clipboard.setData(ClipboardData(text: trip.shareLink)); if (dialogContext.mounted) ScaffoldMessenger.of(dialogContext).showSnackBar(const SnackBar(content: Text('Trip invitation link copied.'))); }, icon: const Icon(Icons.copy_outlined), label: const Text('Copy link')),
      FilledButton.icon(onPressed: () { Navigator.pop(dialogContext); showModalBottomSheet<void>(context: context, isScrollControlled: true, builder: (_) => const FriendManagementSheet()); }, icon: const Icon(Icons.person_add_outlined), label: const Text('Invite friends')),
    ],
  ),
);
...
String _activityKey(String tripId, int day, int index, String title) => '$tripId-$day-$index-$title';

Future<void> _showActivityForm(BuildContext context, CollaborationPreviewViewModel viewModel, {bool proposal = false, PreviewActivity? existing, String? activityKey}) async {
...
Future<void> _confirmPin(BuildContext context, CollaborationPreviewViewModel viewModel, String activityKey) => showDialog<void>(context: context, builder: (dialogContext) => AlertDialog(title: const Text('Pin activity'), content: const Text('Pin to top of itinerary?'), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('No')), FilledButton(onPressed: () { viewModel.toggleActivityPin(activityKey); Navigator.pop(dialogContext); }, child: const Text('Yes'))]));
