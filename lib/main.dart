...
void main() => runApp(const GoBuddyApp());
...
class GoBuddyHome extends StatefulWidget {
  const GoBuddyHome({super.key});
  @override
  State<GoBuddyHome> createState() => _GoBuddyHomeState();
}

class _GoBuddyHomeState extends State<GoBuddyHome> {
  AppPage page = AppPage.discover;
  int tab = 0;
  String filter = 'All';
  final Set<String> styles = {'Adventure', 'Nature'};

  void go(AppPage value) => setState(() => page = value);
  void goTab(int value) => setState(() {
        tab = value;
        page = value == 0
            ? AppPage.discover
            : value == 1
  ...
  Widget build(BuildContext context) {
    final content = switch (page) {
      AppPage.discover => DiscoverPage(
          ...
          onGo: go),
      AppPage.filters => FilterPage(
          onBack: () => go(AppPage.discover),
          onApply: () => go(AppPage.discover)),
      AppPage.details => TripDetailsPage(
          onBack: () => go(AppPage.discover),
          onRequest: () => go(AppPage.request)),
      AppPage.create => CreateTripPage(
          onBack: () => go(AppPage.discover),
          onPublish: () => go(AppPage.myTrips)),
      AppPage.edit => CreateTripPage(
          edit: true,
          onBack: () => go(AppPage.myTrips),
          onPublish: () => go(AppPage.myTrips)),
      AppPage.myTrips => MyTripsPage(
          onCreate: () => go(AppPage.create),
          onManage: () => go(AppPage.manage),
          onEdit: () => go(AppPage.edit)),
      AppPage.request => RequestPage(
          onCancel: () => go(AppPage.details), onSend: () => go(AppPage.sent)),
      AppPage.sent => RequestSentPage(onBack: () => go(AppPage.discover)),
      AppPage.manage =>
        ManageRequestsPage(onApplicant: () => go(AppPage.applicant)),
      AppPage.applicant => ApplicantPage(onBack: () => go(AppPage.manage)),
      AppPage.profile => const ProfilePage(),
    };
    final navPages = {AppPage.discover, AppPage.myTrips, AppPage.profile};
    return Scaffold(
      body: SafeArea(child: content),
      bottomNavigationBar:
          navPages.contains(page) ? BottomNav(tab: tab, onTap: goTab) : null,
      floatingActionButton: page == AppPage.discover
          ? FloatingActionButton(
              onPressed: () => go(AppPage.create),
        ...
        child: Column(children: [
          Padding(
              ...
              child: Row(children: [
                ...
                const Spacer(),
                IconButton(
                    onPressed: () => onGo(AppPage.filters),
          ...
          SizedBox(
            ...
            child: ListView(
              ...
              children:
                  ['All', 'Adventure', 'Culture', 'Luxury', 'Nature', 'Foodie']
                      .map(
                        (x) => Padding(
          ...
          Expanded(
              child: ListView(
                  ...
                  children: [
                TripCard(
                    onDetails: () => onGo(AppPage.details),
                    onRequest: () => onGo(AppPage.request)),
...
class CreateTripPage extends StatelessWidget {
  ...
  const CreateTripPage(
      {super.key, required this.onBack, required this.onPublish, this.edit = false});
  @override
  Widget build(BuildContext context) => FormPage(
          ...
          child:
            ...
            const SizedBox(height: 18),
             Row(children: [
              Expanded(
                  child: AppField(
                      hint: edit ? '12/08/2025' : 'dd/mm/yyyy', icon: Icons.calendar_today_outlined)),
              SizedBox(width: 10),
              Expanded(
                  child: AppField(
                      hint: edit ? '20/08/2025' : 'dd/mm/yyyy', icon: Icons.calendar_today_outlined))
            ]),
            const SizedBox(height: 18),
            const FieldLabel('BUDGET'),
            AppField(hint: edit ? '\$1,200 – \$1,800' : 'e.g. \$1,200 – \$1,800'),
            const SizedBox(height: 18),
            const FieldLabel('TRAVEL STYLE'),
            const StyleWrap(),
            const SizedBox(height: 18),
            ...
            AppField(
                hint: edit ? 'Looking for a calm travel companion to explore Tokyo’s neighbourhoods, hidden izakayas, ramen shops, and art museums.' : 'Describe your trip and ideal companion...', lines: 4),
            const SizedBox(height: 24),
            PrimaryButton(label: edit ? 'Save Changes' : 'Publish Trip', onTap: onPublish),
            if (edit) ...[
              const SizedBox(height: 10),
...
class MyTripsPage extends StatelessWidget {
  ...
  const MyTripsPage(
      {super.key, required this.onCreate, required this.onManage, required this.onEdit});
  @override
  Widget build(BuildContext context) =>
      ListView(padding: const EdgeInsets.fromLTRB(20, 18, 20, 100), children: [
        ...
        const SizedBox(height: 24),
        CompactTrip(destination: 'Tokyo, Japan', dates: 'Aug 12 – Aug 20, 2025', members: '1 joined', onEdit: onEdit, onManage: onManage),
        const SizedBox(height: 14),
        CompactTrip(destination: 'Bali, Indonesia', dates: 'Sep 5 – Sep 15, 2025', members: '2 joined', image: _bali),
        const SizedBox(height: 14),
        CompactTrip(destination: 'Paris, France', dates: 'Oct 1 – Oct 8, 2025', members: '0 joined', image: _paris, status: 'Closed'),
        const SizedBox(height: 14),
        CompactTrip(destination: 'Kyoto, Japan', dates: 'Nov 10 – Nov 18, 2025', members: '1 joined', image: _kyoto, status: 'Draft'),
        const SizedBox(height: 24),
...
class ManageRequestsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      ListView(padding: const EdgeInsets.fromLTRB(20, 18, 20, 30), children: [
        ...
        const SizedBox(height: 14),
        const ApplicantCard(name: 'Lucas Mendes', letter: 'LM', status: 'Accepted'),
        const SizedBox(height: 14),
          ...
          Container(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    const Center(
                        child: Avatar(
                            letter: 'PS', size: 80, color: _violet)),
                    const SizedBox(height: 12),
