...
on public.matchmaking_profiles for select to authenticated
using (true);

create policy "users can insert their matchmaking profile"
on public.matchmaking_profiles for insert to authenticated
with check (id = (select auth.uid()));

create policy "users can update their matchmaking profile"
on public.matchmaking_profiles for update to authenticated
...
as $$
begin
  insert into public.matchmaking_profiles (id, display_name)
  values (
    new.id,
    coalesce(
...
grant select, insert, update on public.matchmaking_profiles to authenticated;
